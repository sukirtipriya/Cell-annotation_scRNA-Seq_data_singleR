...
# script to annotate cell type --scRNA
...
# Input Data 10X CellRanger.HDFS format ---------------------------

hdfs_obj <- Read10X_h5(filename ='xxxxxxxxxxxxxxxx.h5',
                       use.names = TRUE,
                       unique.features = TRUE)

pbmc.seurat <- CreateSeuratObject(counts = hdfs_obj) 
...
# It is a good practice to filter out cells with non-sufficient genes identified 
# and genes with non-sufficient expression
 ...
 # run singleR using combine ref
 # saving counts into a separate object 
 
 pbmc_counts <- GetAssayData(pbmc.seurat.filtered, slot = 'counts')
 
 cum.res1 <- SingleR(test = pbmc_counts, ref = combined, labels = combined$label.main)
 
 table(com.res1$labels)
 
 pbmc.seurat.filtered$com.res1.labels <- com.res1[match(rownames(pbmc.seurat.filtered@meta.data),
                     rownames(com.res1)), 'labels']
                     
view(pbmc.seurat.filtered@meta.data)

DimPlot(pbmc.seurat.filtered, reduction = 'umap', group.by = 'com.res1.labels',
         label = TRUE)
...
grouping <- paste0(com.res.res2$labels, '.',
                    com.res2$reference) 
best_ref <- as.data.frame(split(com.res2,
                   grouping)) 

# get de. genes from individual references

metadata(com.res2$orig.results$HPCA)$de.genes
metadata(com.res2$orig.results$DICE)$de.genes

#...3. strategy 3: using hormonized labels--------------------------------
